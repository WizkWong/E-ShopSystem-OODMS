...
import com.mycompany.oodms.Component.EditProductForm;
...
public class AdminProductPage extends javax.swing.JPanel {
    ...
    public AdminProductPage() {
        ...
        ModifyProBut.setEnabled(false);
        DeleteProBut.setEnabled(false);
    }
    ...
    private void initComponents() {
        ...
        ModifyProBut = new javax.swing.JButton();
        ...
        DeleteProBut.setFont(new java.awt.Font("Segoe UI", 0, 18)); // NOI18N
        DeleteProBut.setText("Delete Product");
        DeleteProBut.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                DeleteProButActionPerformed(evt);
            }
        });
        add(DeleteProBut, new org.netbeans.lib.awtextra.AbsoluteConstraints(1000, 730, 170, 40));

        ModifyProBut.setFont(new java.awt.Font("Segoe UI", 0, 18)); // NOI18N
        ModifyProBut.setText("Modify Product");
        ModifyProBut.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ModifyProButActionPerformed(evt);
            }
        });
        add(ModifyProBut, new org.netbeans.lib.awtextra.AbsoluteConstraints(810, 730, 170, 40));
    ...
    private void productTableMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_productTableMousePressed
        ModifyProBut.setEnabled(true);
        DeleteProBut.setEnabled(true);
    ...
    private void ModifyProButActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ModifyProButActionPerformed
        // Get selected row
        int select = productTable.getSelectedRow();
        
        // Take the product ID of the selected row
        long id = (long) productTableModel.getValueAt(select, 0);

        // Get every data of the selected product ID
        Item item = itemDao.searchId(id);

        // Double validation if system have any error
        if (item == null) {
            JOptionPane.showMessageDialog(null, "Error", "Product does not exist", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        // Run the edit product form
        EditProductForm EditProductform = new EditProductForm(item);
        
        while (true) {
            int option = JOptionPane.showConfirmDialog(null, EditProductform, "Edit Product", JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);
            ...
            // Check validation by using the method EditProductform.formValidate
            if (EditProductform.formValidate(item)) {
                JOptionPane.showMessageDialog(null, "Successfully Updated Product", "Success", JOptionPane.INFORMATION_MESSAGE);
                // Refresh Page
                OODMS.frame.refresh(new AdminProductPage());
                break;
            } else {
                JOptionPane.showMessageDialog(null, "Failed to Updated Product", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
        
    }//GEN-LAST:event_ModifyProButActionPerformed
    ...
    private void DeleteProButActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_DeleteProButActionPerformed
        ...
        int option = JOptionPane.showConfirmDialog(null, "Confirm on deleting product?", "Delete Product", JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);
        if (option != JOptionPane.OK_OPTION) {
            return;
        }
        ...
        if (itemDao.remove(item)) {
            JOptionPane.showMessageDialog(null, "Successfully Deleted Product", "Success", JOptionPane.INFORMATION_MESSAGE);
            // Refresh Page
            OODMS.frame.refresh(new AdminProductPage());
        } else {
    ...
    private javax.swing.JButton ModifyProBut;
